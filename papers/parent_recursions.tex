...
\title{Parent Recursions: Recovering $c^2 = c + 1$\\
...
\begin{abstract}
The Unified Mechanics axiom $c^2 = c + 1$ is standardly taken as a first
...
Their interface produces $x = 1 + 1/x$, equivalent to $c^2 = c + 1$,
...
produce $c^2 = c + 1$ as the unique stable interface recursion?}
\end{quote}
...
transformations. The axiom $c^2 = c + 1$ is not a first principle.
...
The UM axiom $c^2 = c + 1$ is not a first principle. It is the
